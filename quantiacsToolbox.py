...
def loadData(marketList=None, dataToLoad=None, refresh=False, beginInSample=None, endInSample=None, dataDir = 'tickerData'):
    ...
    sampleRange = [int((beginInSample +
                   datetime.timedelta(endLoop)).strftime('%Y%m%d')) for
                   endLoop in range((endInSample-beginInSample).days+1)]

    largeDateRange = range(datetime.datetime(1990, 1, 1).toordinal(),
                           datetime.datetime.today().toordinal() + 1)
    # largeDateRange = range(datetime.datetime(1990,1,1).toordinal(),endInSample.toordinal()+1)
    ...
    dataDict['DATE'] = dataDict['CLOSE'].loc[:endInSampleInt, :].index.values
